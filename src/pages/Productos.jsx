...
import { supabase } from '../supabaseClient' // ajustá la ruta según tu estructura
  ...
  return (
    <section className="relative min-h-screen bg-[#0a0a0a] px-8 py-12 font-stencil text-[#c9b037] tracking-widest">
      <div className="absolute inset-0 pointer-events-none z-0">
        ...
        <div className="absolute top-1/2 left-1/2 w-[600px] h-[600px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#c9b037] opacity-10 animate-pulse-radar" style={{ animationDelay: '1.5s' }}></div>
      </div>
      ...
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 max-w-7xl mx-auto">
        {productos.map((producto) => (
          <div key={producto.id} className="bg-[#1c1c1c] border-2 border-[#c9b037] rounded-lg shadow-lg p-6 flex flex-col items-center hover:scale-[1.04] transition-transform duration-300">
            <img src={producto.imagen} alt={producto.nombre} className="w-full h-44 object-cover rounded-md border border-[#7a6e1d] mb-4" />
            <h3 className="text-xl font-bold mb-2 text-[#f9f9d1] tracking-widest">{producto.nombre}</h3>
            <p className="text-sm italic text-[#d9d7a1] mb-3 text-center whitespace-pre-wrap">{producto.descripcion}</p>
            ...
            <ul className="text-sm text-[#d9d7a1] mb-4 space-y-1 w-full">
              {producto.sabores && (
                <li>
                  ...
                  {Array.isArray(producto.sabores) ? producto.sabores.join(', ') : producto.sabores}
                </li>
              )}
              {producto.colores && (
                <li>
                  <span className="font-semibold text-[#f9f9d1]">Colores:</span>{' '}
                  {Array.isArray(producto.colores) ? producto.colores.join(', ') : producto.colores}
                </li>
              )}
            </ul>
